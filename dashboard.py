...
def add_markdown_toolbar() -> None:
    ...
    cols = st.columns(5)
    for idx, (name, snippet) in enumerate(presets.items()):
        if cols[idx % 5].button(name, key=f"fmt-{name}"):
            current = st.session_state.get("compose_message", "")
            st.session_state.compose_message = (current + "\n\n" + snippet).strip()
...
def new_message_tab(people: list[str]) -> None:
    ...
    message = st.text_area("Message (Markdown)", key="compose_message", height=260, placeholder="Write your markdown message here...")
    ...
    if st.button("Send", type="primary"):
        ...
        st.session_state.compose_message = ""
