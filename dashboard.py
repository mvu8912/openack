    ...
    for person_dir in sorted(path for path in MESSAGES_ROOT.iterdir() if path.is_dir()):
        ...
        if inbox.exists():
            for msg_path in sorted(inbox.glob("*.md"), reverse=True):
                ...
                records.append(
                    MessageRecord(message_id, f"{person_dir.name}/inbox", True, details.sent_at, details.sender, details.recipient, _message_preview(details.body), len(details.attachments))
                )
        ...
        if done.exists():
            for zip_path in sorted(done.glob("*.zip"), reverse=True):
                with zipfile.ZipFile(zip_path, "r") as archive:
                    ...
                    detail_cache[message_id] = details
                    records.append(
                        MessageRecord(message_id, f"{person_dir.name}/done", False, details.sent_at, details.sender, details.recipient, _message_preview(details.body), len(details.attachments))
                    )
...
def require_login() -> None:
    ...
    if submitted:
        if username == ADMIN_USER and expected_password and password == expected_password:
...
def inbox_tab(records: list[MessageRecord], detail_cache: dict[str, MessageDetails]) -> None:
    st.subheader("Inbox")
    show_all = st.checkbox("Show all participants", value=False)
    visible = records if show_all else [item for item in records if ADMIN_USER in {item.sender, item.recipient}]
...
def new_message_tab(people: list[str]) -> None:
    ...
    message = st.text_area("Message (Markdown)", height=240, placeholder="Write your markdown message here...")
